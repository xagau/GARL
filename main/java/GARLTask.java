    ...
    public static GARLRectangle closest(ArrayList<GARLRectangle> list, Entity e) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            //if (g.getCenterX()+g.width/2 < (e.location.x+e.size/2) && (g.getCenterY()+g.height/2 < e.location.y+e.size/2)) {
                int tdistX = (int) (g.getCenterX()+g.width/2) - ((int)e.location.x+e.size/2);
                int tdistY = (int) (g.getCenterY()+g.height/2) - ((int)e.location.y+e.size/2);
                    ...
                    try {
                        if( target == false ){
                            ...
                            if( isTrajectoryDeath() ) {
                                genome.advance();
                                Action a = Action.RECODE; //ActionFactory.create(Math.random());
                                process(Action.SLOW, world, depth);
...
class Settings {
    ...
    static int STARTING_POPULATION = 50;
    static int MAX_OFFSPRING = 8;
    ...
    final static double ENERGY_STEP_SLEEP_COST = 0.01;
